...
public partial class MainWindow : Form
{
    ...
    private void btnChangeAmount_Click(object sender, EventArgs e)
    {
        ...
        UpdateAmount update = new UpdateAmount("Change transaction amount", "Transaction amount: ", selected.Amount.ToString("N2"));
        if (update.ShowDialog() == DialogResult.OK)
        {
            using (SqlConnection cxn = new SqlConnection(cxnStr))
            {
    ...
    {
        Transaction selected = (Transaction)lstbTransactions.SelectedItem;
        UpdateTypeMessageBox update = new UpdateTypeMessageBox(selected.Type);
